...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Modified by: Brandon Davis
% EEL5934 Marine Vehciles Project
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clc;
clear;
close all;
...
sim_filepath = "../2023-11-17-15-11-06/";
accel_csv = readtable("2023-11-17-15-11-06\accel-0.csv").Variables;
mag_csv = readtable("2023-11-17-15-11-06\mag-0.csv").Variables;
gpstime_csv = readtable("2023-11-17-15-11-06\gps_time.csv").Variables;
gyro_csv = readtable("2023-11-17-15-11-06\gyro-0.csv").Variables;
gps_csv = readtable("2023-11-17-15-11-06\gps-0.csv").Variables;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% USER INPUTS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
f_s    = 100;   % sampling frequency [Hz]
f_gnss = 0.5;     % GNSS measurement frequency [Hz]
...
for i=1:N+1
    ...
    if mod( t, h_gnss ) == 0
        ...
        if t > 0
            %Error measurement
            j = i-1;
            estimate_0 = simdata(j,17:19);  %Last position estimate
            diff = norm(y_pos' - estimate_0);
            error = [error; t, diff];
        end
...
h1 = plot3(y_m(:,1),y_m(:,2),y_m(:,3),'xk'); hold on;
h2 = plot3(x_hat(:,1),x_hat(:,2),x_hat(:,3),'m'); hold off;
...
%Error handling
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
total_dist = norm(ydata(end,2:4)-ydata(1,2:4));
t_error = error(:,1);
final_error = error(:,2)/total_dist;
total_error = norm(final_error)

% Plot error
figure(4); figure(gcf)
h1 = plot(t_error, error(:,2),'xb');
xlabel('time (s)'),ylabel('error (m)'),title('Position error'),grid
legend([h1(1)],['Position Update'])
